...
export default function MovieDetails() {
	...
	if (isDeleted) return (
		<div className="p-6 flex flex-col gap-4">
			...
			<Link to="/" className="w-fit border rounded-lg p-2 bg-[#f2f0e9] text-sm">← Back to Watchlist</Link>
		</div>
	);
	if (isError || !movie) return <p className="text-red-400 p-6">Movie not found.</p>;

	return (
		<div className="p-6">
			<div className="flex flex-row items-center gap-2 mb-4">
				<Link to="/" className="border rounded-lg m-1 p-2 bg-[#f2f0e9]">
					← Back
				</Link>
				<p className="text-white">Movie Detail</p>
			</div>
			...
			<div className="flex flex-col gap-5">
				<section className="bg-[#FAF9F6] flex items-center gap-4 p-4 mt-4 rounded-lg">
					...
					<div className="flex flex-col gap-1 flex-1">
						<h1 className="text-xl font-semibold text-[#480902]">{movie.title}</h1>
						<p className="text-sm text-gray-500 capitalize">{movie.genre}</p>
						<span className={`text-xs text-white capitalize rounded-xl p-1 px-2 w-fit ${movie.status === 'want to watch' ? 'bg-[#CE793A]' : 'bg-[#480902]'}`}>
							{movie.status}
						</span>
					</div>
				...
				<section className="bg-[#FAF9F6] rounded-lg p-5">
					...
					<div className="flex flex-col divide-y divide-stone-200">
						<div className="flex items-center py-3 px-2">
							<span className="text-sm text-gray-400 w-24 shrink-0">Status</span>
							<span className={`text-xs text-white capitalize rounded-xl py-1 px-2 ${movie.status === 'want to watch' ? 'bg-[#CE793A]' : 'bg-[#480902]'}`}>
								{movie.status}
							</span>
						</div>
						<div className="flex items-center py-3 px-2">
							<span className="text-sm text-gray-400 w-24 shrink-0">Rating</span>
							<span className="text-sm font-medium text-[#480902]">
								{movie.rating ? `${movie.rating} / 5` : '—'}
							</span>
						</div>
						<div className="flex items-center py-3 px-2">
							<span className="text-sm text-gray-400 w-24 shrink-0">Genre</span>
							<span className="text-sm capitalize">{movie.genre || '—'}</span>
						</div>
						<div className="flex items-start py-3 px-2">
							<span className="text-sm text-gray-400 w-24 shrink-0">Notes</span>
							<span className="text-sm text-gray-600">{movie.notes || '—'}</span>
						</div>
					</div>
				</section>
			</div>
